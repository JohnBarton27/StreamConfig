...
if args.input_number == "1" and args.cam_name == "PTZ1":
    set_channel_url = f"http://{ip_addr}/mwapi?method=set-channel&ndi-name=false&name={urllib.parse.quote('CAM 201 (Chan2 192.168.2.201)')}"
elif args.input_number == "2" and args.cam_name == "PTZ2":
    set_channel_url = f"http://{ip_addr}/mwapi?method=set-channel&ndi-name=false&name={urllib.parse.quote('CAM 200')}"
else:
